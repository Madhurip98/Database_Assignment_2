...
SELECT ProductName FROM products LIMIT 4, 14;
...
Select CustomerName, CustomerID, Address, City, PostalCode, Country from customers where Country<>"USA" AND Country<>"Canada";
...
SELECT O.OrderDetailID,O.OrderID, O.ProductID, O.Quantity, A.OrderDate
FROM orders A
INNER JOIN order_details O
ON A.OrderID = O.OrderID
where year (A.OrderDate) between 1997 and 1998
order by O.OrderID DESC;

-- The years column is not there in the order_details table so couldn't complete question 7 as advised
...
SELECT EmployeeID, LastName, FirstName, BirthDate, Photo, Notes FROM employees WHERE FirstName != 'Sanjay' and FirstName != 'Sonia';
